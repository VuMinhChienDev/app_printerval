...
// Định nghĩa props để nhận callback tính tổng tiền
interface OrderReviewProps {
  onTotalCalculated?: (total: number) => void; // Callback để truyền tổng tiền lên cha
}
...
export default function OrderReview({ onTotalCalculated }: OrderReviewProps) {
  const params = useLocalSearchParams();
  const cartItems = params.cartItems && typeof params.cartItems === 'string' ? JSON.parse(params.cartItems) : [];
  ...
  const shippingCost = 7.99; // Phí vận chuyển cố định
  const additionalCost = 2.99; // Phí tải xuống thiết kế

  // Hàm tính tổng tiền
  const calculateTotal = () => {
    let subtotal = 0;
    itemsState.forEach((item: { productImage: any; quantity: number; isChecked: any; }, index: string | number) => {
      if (item.productImage) {
        const product = cartItems[index];
        const baseTotal = product.price * item.quantity;
        const itemTotal = item.isChecked ? baseTotal + additionalCost : baseTotal;
        subtotal += itemTotal;
      }
    });
    const total = subtotal + shippingCost; // Cộng phí vận chuyển
    if (onTotalCalculated) {
      onTotalCalculated(total); // Gửi tổng tiền lên cha
    }
    return total;
  };
  ...
  const handleAddImage = (id: any) => {
    setItemsState((prev: any[]) => {
      ...
      calculateTotal(); // Tính lại tổng tiền sau khi thêm
      return newState;
    });
  };
  ...
  const handleQuantityChange = (id: any, delta: number) => {
    setItemsState((prev: any[]) => {
      ...
      calculateTotal(); // Tính lại tổng tiền khi số lượng thay đổi
      return newState;
    });
  };
  ...
  // Xử lý chọn/tắt tải xuống thiết kế
  const handleCheckToggle = (id: any) => {
    setItemsState((prev: any[]) => {
      const newState = prev.map((item) =>
        item.id === id ? { ...item, isChecked: !item.isChecked } : item
      );
      calculateTotal(); // Tính lại tổng tiền khi chọn tải xuống
      return newState;
    });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {cartItems.map((product: { price: number; title: any; subtitle: any; originalPrice: number }, index: any) => {
        const itemState = itemsState[index];
        if (!itemState.productImage) return null;
